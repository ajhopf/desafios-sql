with salario_empregado as (
    ...
    group by ev.matr, d.desconto, e.nome, e.lotacao_div
     ...
     media_departamento as (select dep.nome as departamento, div.nome as divisao, round(avg(salario_empregado.salario),2) as media
     ...
     ranked_divisoes AS (
         SELECT
             ...
             divisao,
             media,
             ROW_NUMBER() OVER (PARTITION BY departamento ORDER BY media DESC) AS rn
         FROM media_departamento
     )
SELECT departamento, divisao, media
FROM ranked_divisoes
WHERE rn = 1
order by media desc;